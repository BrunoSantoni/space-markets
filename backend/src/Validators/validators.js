...
const supermarketValidatorCreate = Joi.object().keys({
  ...
  market_mail: Joi.string().required().email().error(new Error('E-mail deve ser válido')),
  market_password: Joi.string().min(6).max(20).error(new Error('Senha deve conter entre 6 e 20 caracteres')),
  ...
  market_number: Joi.number().required().error(new Error('Informe um número para o endereço')),
